...
func (xmlRule *ResponseRule) MapToRule() *Rule {
	...
	return &Rule{IsActive: xmlRule.Enabled, URLPattern: match, MatchType: matchType, Latency: xmlRule.Latency, Method: "*"}
}

//MapToResponse Transforms ResponseRule to Response
func (xmlRule *ResponseRule) MapToResponse() *Response {
	if xmlRule.Action != "" {
		//Action might point out the directly a response file
		resp := xmlRule.mapToResponseUsingFile()

		if resp != nil {
			return resp
		} else if xmlRule.Headers == "" {
			return nil
		}
	}
...
func (xmlRule *ResponseRule) mapToResponseUsingFile() *Response {
	content, err := ioutil.ReadFile(xmlRule.Action)
	...
	return NewResponseFromString(string(content))
}
